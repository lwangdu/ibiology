...
jQuery( document ).ready( function( $ ) {
	...
	$('.fca_eoi_width_input').on('change input', function() {
		...
		$target.css('width', $(this).val() + units )
	})
	...
	$('#fca_eoi_layout_select_button').click(function() {
		...
		if ( layout_id.indexOf('lightbox') !== -1 ) {
			$( '#layouts_types_tabs').find('a').eq(0).click()
		} else if ( layout_id.indexOf('postbox') !== -1 ) {
			$( '#layouts_types_tabs').find('a').eq(1).click()
		} else if ( layout_id.indexOf('banner') !== -1 ) {
			$( '#layouts_types_tabs').find('a').eq(3).click()
		} else if ( layout_id.indexOf('overlay') !== -1 ) {
			$( '#layouts_types_tabs').find('a').eq(4).click()
		} else {
			$( '#layouts_types_tabs').find('a').eq(2).click()
		}
	})
	...
	$('[name="fca_eoi[show_name_field]"]').change(function() {
		...
		if ( !this.checked ) {
			$('#accordion-info-name_field').html( hidden_icon )
			if (  layout_id.indexOf('overlay') === -1 && layout_id.indexOf('banner') ) {
				$( $('[name="fca_eoi[email_width_selector]"]').val() ).css('margin-left', 'initial')
				$( $('[name="fca_eoi[email_width_selector]"]').val() ).css('margin-right', 'initial')
				$( $('[name="fca_eoi[email_width_selector]"]').val() ).css('float', 'none')
			}

		} else {
			$('[name="fca_eoi[name_placeholder]"]').keyup()
			
			if ( layout_id.indexOf('layout') === -1 && layout_id.indexOf('overlay') === -1 && layout_id.indexOf('banner') === -1 ) {
				$('[name="fca_eoi[name_width]"]').next().val('%')
				$('[name="fca_eoi[name_width]"]').val('49').change()
				$('[name="fca_eoi[email_width]"]').next().val('%')
				$('[name="fca_eoi[email_width]"]').val('49').change()
			}
			
			if (  layout_id.indexOf('overlay') === -1 && layout_id.indexOf('banner') ) {
				$( $('[name="fca_eoi[email_width_selector]"]').val() ).css('margin-left', 'initial')
				$( $('[name="fca_eoi[email_width_selector]"]').val() ).css('margin-right', 'initial')
				$( $('[name="fca_eoi[email_width_selector]"]').val() ).css('float', 'right')
			}
		}
